...
RUN APP_URL="http://testingshopware.myserver.fraettstreet.pro" DATABASE_URL="" php bin/console assets:install \
    && rm -Rf var/cache \
    && touch install.lock \
    && mkdir -p var/cache
...
HEALTHCHECK --timeout=10s CMD curl --silent --fail http://testingshopware.myserver.fraettstreet.pro:8000/fpm-ping
